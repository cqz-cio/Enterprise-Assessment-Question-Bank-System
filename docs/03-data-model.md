...
## 候选人导入清单归档 V025（2026-09-24，D-039）

替代 V024 的内存任务保存方式，新增 `el_candidate_import_task`，不修改已有分配/试卷/凭证摘要。

| 列 | 类型 | 用途 |
| --- | --- | --- |
| id | VARCHAR(36)，主键 | 随机任务 ID |
| owner_id | VARCHAR(64) | 创建账号 ID，所有恢复/关闭/提交均匹配 |
| file_hash | VARCHAR(64) | 原 XLSX 的 SHA-256，配合 owner 恢复同一文件 |
| committed | TINYINT | 是否处理完全部有效行 |
| expires_at / created_at | BIGINT | 毫秒时间戳，预览 15 分钟、开始处理后 30 天 |
| payload | LONGTEXT | v1 格式的 AES-256-GCM 密文，含文件名、原始行及发放结果 |

索引为 (owner_id,created_at)、(owner_id,file_hash,expires_at)、(expires_at)。载荷采用随机 12 字节 nonce、128 位认证标签；AAD 绑定版本、任务、所有者、文件摘要、到期时间及完成标记，防止调包。加密密钥通过既有 ASSIGNMENT_CODE_PEPPER 的独立 HMAC 域派生；密钥不落库，必须保留原值，轮换前需单独迁移归档。本轮不轮换密钥。

同任务逐行 SELECT FOR UPDATE；候选人创建、防重键和本行归档同事务提交。归档失败整行回滚，不会发放后丢失码。已成功/部分成功任务关闭仍保留，到期按分钟清理。导出前重查当前权限及凭证状态，重置/停用/过期码不返回。密文归档是 D-039 允许的独立导出用途，不改变认证表只保存摘要和日志不得含明文的规则。
