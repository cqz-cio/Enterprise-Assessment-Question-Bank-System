...
复用已批准的透明品牌标志，以 30px 图标形式显示；保留系统名称，自定义 Logo 兼容，收起时居中。组件 ESLint、完整类型检查、10 项真实组件 Chrome 场景和首页导航检查通过；正式构建通过，CI/CD 发布跟进中。无接口、迁移或依赖变更，详见 [交付记录](./SIDEBAR_LOGO_DELIVERY.md)。
