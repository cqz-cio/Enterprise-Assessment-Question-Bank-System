...
- [x] 清理原项目名称、Logo、外部宣传和演示文案（D-034，2026-09-20；保留内部兼容命名和许可证）。
- [ ] 关闭或限制生产 Swagger。
- [ ] 前端生产构建。
...
已清理旧名称、公司页脚/官网、Logo/favicon、远程默认头像、登录介绍、API 文档、README 与演示记录名称；修正配置缓存和会话展示刷新，同步后端自带静态前端。V022/V023 已应用。后端 105 项测试、前端构建/lint、MySQL 迁移保护和幂等验证、真实浏览器检查通过；全库 197 个文字字段无残留，核心业务内容保持不变。修改文件、备份、保留项和限制见 `BRANDING_CLEANUP_DELIVERY.md`。下一步候选人 Excel 导入。


## 候选人 Excel 批量导入完成（2026-09-20）

- 按批准预览实现三步弹窗、标准模板、逐行预校验、部分成功发放、问题行报告、考核码下载及离开提醒。
- V024 已应用；数据库并发防重覆盖手工及批量发放，任务绑定创建者且有容量/时间限制。详情见 `P1_CANDIDATE_IMPORT_DELIVERY.md`、D-035 和 API 4.2。
- 后端 114 项测试通过；前端正式构建、新增文件 lint 通过；完整类型检查仍有 32 项既有问题，新增文件无类型错误。
- 真实 HTTP/MySQL 验证 4 成功/1 错误/1 重复、未登录/员工/候选人/跨用户拒绝、4 并发同任务幂等、2 任务争抢同一编号仅创建一次、Excel 行数及考核码认证。浏览器验证上传、校验、筛选、提交和离开提醒，下载按钮触发成功；浏览器下载事件确认超时，文件内容由实际接口返回的 XLSX 验证。
- 临时数据和会话已清理，八张表恢复原计数。下一项建议生产部署/备份恢复专项；前端旧类型债务继续保留。

## Windows 运维准备（D-036，2026-09-21）

- 用户确认先完善当前 Windows，最终服务器尚未确定。
- 已完成可选 prod 配置、文档入口关闭、日志轮转、独立发布构建与 JAR 前端一致性检查，以及数据库/附件备份与隔离恢复工具。
- 后端 115 项、运维 13 项测试和正式发布构建通过；40 表/685 行/V024/3 附件恢复演练通过。prod 在独立数据库和 Redis 下启动，站点可用且文档入口 404。
- 无新增迁移和业务 API；不切换当前业务环境的配置。真实服务器 HTTPS、端口隔离、认证、备份计划和异机密钥托管待落实。详见 `P1_WINDOWS_OPS_DELIVERY.md`、`deploy/windows/README.md`。

## Linux 测试环境（D-037，2026-09-21）

- 完整数据恢复到独立 MySQL/Redis，测试站使用用户明确选定的新密钥。安装 Java 17 systemd 服务、IP HTTPS、独立证书续期及私有备份。
- 发布构建 115 后端测试/137 前端文件校验通过；40 表/685 行/V024/3 附件恢复校验通过；服务器内部真实考核、权限、并发、过期、代理防伪及备份验证通过，临时数据已清理。
- 公网 18443 等待腾讯云入站规则放行；服务器浏览器业务验收、压力测试、业务备份调度/异机副本仍待落实。无业务 API 或新迁移，详见 `P1_LINUX_TEST_DELIVERY.md`。
...
## 公司日常使用全链路验收（2026-09-24）

新增可重复的隔离验收入口 `scripts/qa/run_acceptance.py`，真实配置、注册审批、发放、答题、阅卷、结果、导入/导出、权限、过期与并发测试通过。修复默认立即生效时间的 DATETIME 舍入问题；后端 117 项、接口/业务 100 项、Chrome 12 个场景及保存队列专项通过。100 人并发无请求错误，数据/资源自动清理核对通过；完整证据见 [验收报告](./DAILY_USE_ACCEPTANCE_2026-09-24.md)。无新增迁移/API 结构变化，本轮未提交或部署。核心业务具备小范围试运行条件；旧 32 项类型错误、真实服务器完整验收与备份调度继续保留为待办。


## 日常使用问题修复（D-039，2026-09-24）

32 项前端类型错误修复并作为 CI 阻断检查，密码成功提示/重新登录与默认生效时间修正。V025 加密持久保存候选人导入清单，新增恢复 API，关闭/重启/重复上传可找回原结果，逐行事务保证码与分配一致。123 项后端、110 项接口业务、14 项 Chrome 页面场景全部通过，真实 MySQL 重启恢复及源库 40 表不变验证通过；详见 [修复回归报告](./DAILY_USE_FIXES_2026-09-24.md)。备份和服务器容量按用户要求暂缓。

D-039 已于同日通过 GitHub 运行 35950414592 自动部署 2cef942，服务器 Flyway V025、真实业务验收和虚构数据清理均通过；原业务数据校验值未变，公网 Chrome 登录/候选人入口正常。最终证据见修复回归报告。

## 登录与加载页 Logo 透明背景（D-040，2026-09-24）

已按批准效果替换两个位置的内置素材：透明底、登录浅色字样、加载灰色字样，保留页面背景、尺寸和自定义 Logo。正式前端构建、完整类型检查、组件 ESLint、diff 检查及 8 项 Chrome 显示检查通过，包含 1920px 桌面、390/320px 窄屏、深色主题和配置兼容。已推送并通过 CI/CD 运行 35967669039 部署 50d43bd；123 项后端、23 项部署及 6 项公网 Chrome 检查通过，两张图片与源码 SHA256 一致。上传增加限速、保留已收块和一次限时续传。无接口、迁移或依赖变化，下一步刷新测试站确认。详见 [交付记录](./LOGO_TRANSPARENCY_DELIVERY.md) 与根目录 `design-qa.md`。

## 补齐侧栏品牌图标（D-041，2026-09-24）

复用已批准的透明品牌标志，以 30px 图标形式显示；保留系统名称，自定义 Logo 兼容，收起时居中。组件 ESLint、完整类型检查、10 项真实组件 Chrome 场景和首页导航检查通过；正式构建通过。已推送并经 CI/CD 运行 35979337448 成功部署 c98fe7d，123 项后端和 23 项部署测试通过；线上布局代码、透明图片哈希、配置及服务状态核对通过。下一步刷新测试站确认显示。无接口、迁移或依赖变更，详见 [交付记录](./SIDEBAR_LOGO_DELIVERY.md)。


## 全前端品牌显示检查与统一修复（D-042，2026-09-24）

逐一核查登录/注册、首屏、候选人入口、导航、favicon、设置预览及旧默认路径。统一透明图片和独立标志，修复收起状态下顶部布局标题丢失，候选人入口适配深色主题。15 项真实组件状态、路径兼容、快速收起、图标透明像素与首页导航通过；前端 lint、类型与正式构建通过。12 项正式页面复测通过。已推送并经 CI/CD 运行 35982124004 部署 baf7429；123 后端和 23 部署测试、10 项线上 Chrome 页面及 4 项资源哈希核对通过，服务正常。无接口、迁移或依赖变更，详见 [交付记录](./FRONTEND_BRAND_AUDIT_DELIVERY.md)。
