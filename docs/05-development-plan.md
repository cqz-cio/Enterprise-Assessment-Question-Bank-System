...
逐一核查登录/注册、首屏、候选人入口、导航、favicon、设置预览及旧默认路径。统一透明图片和独立标志，修复收起状态下顶部布局标题丢失，候选人入口适配深色主题。15 项真实组件状态、路径兼容、快速收起、图标透明像素与首页导航通过；前端 lint、类型与正式构建通过。12 项正式页面复测通过，CI/CD 发布跟进中。无接口、迁移或依赖变更，详见 [交付记录](./FRONTEND_BRAND_AUDIT_DELIVERY.md)。
