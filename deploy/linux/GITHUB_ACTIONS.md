...
2026-09-24 已清理既有 32 项类型错误；全量 `ts:check` 是阻断检查，失败时不构建发布包、不部署。前端正式构建与后端测试仍必须通过。

CI 使用与项目一致的 pnpm 11.3.0（`packageManager` 已固定），Maven 使用 `scripts/ci/maven-settings.xml` 直连 Maven Central，避免海外 runner 访问默认镜像停滞。本机 Maven 设置保持不变。

**这是单实例维护发布，会短暂停机。** 不要在有人正式答题时发布。首次升级到 V025 前须先下载旧版尚未保存的导入清单；V025 起已发放清单加密持久化，关闭或重启可恢复，保留 30 天，恢复须沿用原 pepper。流水线不重新导入数据库、不改现有密钥、不重新创建容器、不修改 ERP/Nginx/证书。
...
2026-09-21 已完成服务器入口安装、test 环境 Secrets、仓库启用变量和首次真实自动发布，CI/CD 均成功。运行链接、部署版本、备份路径及验收结果见 [真实联调交付记录](../../docs/P1_CICD_DELIVERY.md)。
