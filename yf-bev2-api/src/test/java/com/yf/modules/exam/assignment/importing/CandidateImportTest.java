...
class CandidateImportTest {
    ...
    @BeforeEach void setup() {
        ...
        db.execute("CREATE TABLE el_exam_assignment(id varchar(64),depart_id varchar(64))");
        ...
        service=new CandidateImportService(db,assignments,guard,factory.getValidator());
        ...
        when(assignments.createCandidate(any())).thenAnswer(call->tx.execute(status->{
            ...
            db.update("INSERT INTO el_exam_assignment VALUES(?,?)",req.getCandidateNo(),req.getDepartId());
    ...
    @Test void expiredTaskIsNotAccessible() throws Exception {
        var task=service.preview(file(List.of(row("A"))));
        var field=CandidateImportService.class.getDeclaredField("tasks");field.setAccessible(true);Object internal=((Map<?,?>)field.get(service)).get(task.getTaskId());var expires=internal.getClass().getDeclaredField("expires");expires.setAccessible(true);expires.setLong(internal,0);
        assertThrows(ServiceException.class,()->service.commit(task.getTaskId()));service.cleanup();assertTrue(((Map<?,?>)field.get(service)).isEmpty());
    }
}
